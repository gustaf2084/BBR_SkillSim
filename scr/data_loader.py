...
class GameData:
    def __init__(self, data, path, warnings=None):
        self._data = data
        self._path = path
    ...
    def complete_backgrounds(self):
        return sorted([b for b in self.backgrounds if self.is_background_complete(b)])

    def incomplete_backgrounds(self):
        return sorted([b for b in self.backgrounds if not self.is_background_complete(b)])


def load_data(data_path, expected_version="0.7.6", strict=False):
    ...
    try:
        ...
        raise DataError("JSON error line {} col {}: {}".format(e.lineno, e.colno, e.msg), code="format")
    ...
    if strict and warnings:
        raise DataError("Validation: " + "; ".join(warnings[:5]), code="incomplete", detail=warnings)
    return GameData(raw, data_path, warnings)
